...
numlib::Vector<Ele_T> numlib::gaussian_solver<Ele_T>::operator ()(
    numlib::Matrix<Ele_T,Dense> const &Ain,
    numlib::Vector<Ele_T> b)
{
  ...
  numlib::Matrix<Ele_T,Dense>* A = Ain.clone();
...
numlib::TDMA<Ele_T>::operator ()(
    numlib::Matrix_type<Ele_T> const &Ain,
    numlib::Vector<Ele_T> b)
{
    if(Ain.N() != b.get_size())
    {
    ...
    std::shared_ptr<numlib::Matrix_type<Ele_T>> A = Ain.clone();
    std::size_t N = b.get_size();
